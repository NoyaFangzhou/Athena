...
class ATHSignUpViewController: ATHBaseViewController, ATHInputViewDelegate, ATHFootViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let username = ATHInputView(frame: CGRect(x: 0.0, y: 0, width: SCREEN_WIDTH, height: 80), title: "Username", titleColor: PLACEHOLDER_COLOR, textColor: UIColor.white)
        username.delegate = self
        self.scrollView.addSubview(username)
//        VIEWBORDER(view: username, color: UIColor.orange, width: 2.0)
        let email  = ATHInputView(frame: CGRect(x: 0.0, y: username.y+username.height+5, width: SCREEN_WIDTH, height: 80), title: "Email Address", titleColor: PLACEHOLDER_COLOR, textColor: UIColor.white)
        email.delegate = self
        self.scrollView.addSubview(email)
//        VIEWBORDER(view: email, color: UIColor.orange, width: 2.0)
        let language  = ATHInputView(frame: CGRect(x: 0.0, y: email.y+email.height+5, width: SCREEN_WIDTH, height: 80), title: "Native Language", titleColor: PLACEHOLDER_COLOR, textColor: UIColor.white)
        language.delegate = self
        self.scrollView.addSubview(language)
//        VIEWBORDER(view: language, color: UIColor.orange, width: 2.0)
        let password  = ATHInputView(frame: CGRect(x: 0.0, y: language.y+language.height+15, width: SCREEN_WIDTH, height: 80), title: "Password", titleColor: PLACEHOLDER_COLOR, textColor: UIColor.white)
        password.delegate = self
        self.scrollView.addSubview(password)
//        VIEWBORDER(view: password, color: UIColor.orange, width: 2.0)
        let confirm_pwd  = ATHInputView(frame: CGRect(x: 0.0, y: password.y+password.height+5, width: SCREEN_WIDTH, height: 80), title: "Confirm Password", titleColor: PLACEHOLDER_COLOR, textColor: UIColor.white)
